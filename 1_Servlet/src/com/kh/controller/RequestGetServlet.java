...
public class RequestGetServlet extends HttpServlet {
	...
	protected void doGet(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		 ...
		 *   key값으로 여러개의 value값들을 전달받을 경우(체크박스) 문자열 배열형으로 반환 가능
		 * 
		 */
		 ...
		 * 
		 * int result = new MEmberSevice().insertMember(m);
		 * if(result >0 ) : 성공이라고 보고 commint, 실패시 rollback(Service가 처리)
		 ...
		 * 
		 * 단점 : 복잡, 혹시라도 추후 html을 수정하고자한다면 Java소스코드 내에서 수정이 이루어지기 때문에
		...
		out.println("<span id='name'>"+"이름 : "+name+"</span><br>");
		out.println("<span id='age'>"+"나이 : "+age+"</span><br>");
		out.println("<span id='gender'>"+"성별 : "+gender+"</span><br>");
		out.println("<span id='city'>"+"거주지"+request.getParameter("city")+"</span>");		
